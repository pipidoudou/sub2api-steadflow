...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	logger := handler.ProvideLogger()
	shadowUserService := service.ProvideShadowUserService(client, logger)
	userAPIKeyService, err := service.ProvideUserAPIKeyService(client, logger, configConfig)
	if err != nil {
		return nil, err
	}
	subscriptionAssigner := service.ProvideSubscriptionAssignerAdapter(subscriptionService)
	distributorOrderService := service.ProvideDistributorOrderService(client, shadowUserService, userAPIKeyService, subscriptionAssigner, logger)
	distributorHandler := handler.ProvideDistributorHandler(client, distributorOrderService, userAPIKeyService)
	v := handler.ProvideDistributorAuthMiddleware(client, logger)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, distributorHandler, v, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
