...
class UpgradeCandidateValidationTests(unittest.TestCase):
    ...
    def test_report_uses_generated_paths_after_baseline_reconciliation(self):
        self.release = "v1.2.0"
        manifest_path = self.fixture.fork / ".steadflow" / "customization.yml"
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        manifest["data"]["paths"].append("generated.txt")
        manifest["data"]["paths"].sort()
        manifest["generated"]["paths"] = ["generated.txt"]
        self.fixture.write_json(".steadflow/customization.yml", manifest)
        for root in (self.fixture.fork, self.fixture.official):
            (root / "generated.txt").write_text(
                "shared generated bytes\n", encoding="utf-8"
            )
            self.fixture.run_git("add", "generated.txt", root=root)
        self.fixture.run_git(
            "add", ".steadflow/customization.yml", root=self.fixture.fork
        )
        self.fixture.run_git(
            "commit", "-m", "steadflow generated customization", root=self.fixture.fork
        )
        self.fixture.source_commit = self.fixture.run_git(
            "rev-parse", "HEAD", root=self.fixture.fork
        ).stdout.strip()
        self.fixture.run_git(
            "commit", "-m", "upstream adopts generated bytes", root=self.fixture.official
        )
        self.fixture.add_release(self.release)

        state = create_upgrade_candidate(self.fixture.repository(), self.release)

        report = json.loads(self.report_path().read_text(encoding="utf-8"))
        self.assertEqual(state["phase"], "merged")
        self.assertEqual(report["generated"], {"paths": [], "status": "PASS"})
